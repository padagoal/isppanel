...
class BancosController extends Controller
{
     ...
     * @return mixed
     */
    public function actionView($cuenta)
    {   
        $request = Yii::$app->request; 		
        if($request->isAjax){
            Yii::$app->response->format = Response::FORMAT_JSON;
            return [
                    'title'=> Yii::t('app',$this->title)."# ".$cuenta,
                    'content'=>$this->renderAjax('view', [
                        'model' => $this->findModel($cuenta),
                    ]),
                    'footer'=> Html::button(Yii::t('app','Close'),['class'=>'btn btn-default pull-left','data-dismiss'=>"modal"]).
                            Html::a(Yii::t('app','Edit'),['update','cuenta'=>$cuenta],['class'=>'btn btn-primary','role'=>'modal-remote'])
                ];    
        }else{
        ...
        if($request->isAjax){
            ...
            */
            ...
            if($request->isGet){
                ...
                return [
                    ...
                    'title'=> Yii::t('app', "Add")." ".Yii::t('app', $this->title), 
                    'content'=>'<span class="text-success">Crear Bancos realizado</span>',
                    'footer'=> Html::button(Yii::t('app', 'Close'),['class'=>'btn btn-default pull-left','data-dismiss'=>"modal"]).
            ...
            */
            if ($model->load($request->post()) && $model->save()) {
                return $this->redirect(['updatemd', 'cuenta' => $model->cuenta]);
            } else {
     ...
     * @return mixed
     */
    public function actionUpdate($cuenta)
    {
        $request = Yii::$app->request;  		
        $model = $this->findModel($cuenta);
		if ($model->hasProperty('created_by')) {
        ...
        if($request->isAjax){
            ...
            */
            if ($model->load($request->post()) && $model->save()) {
                return $this->redirect(['view', 'cuenta' => $model->cuenta]);
            } else {
    ...
    /**
     * Delete an existing Bancos model.
     * For ajax request will return json object
     * and for non-ajax request if deletion is successful, the browser will be redirected to the 'index' page.
     * @param string $cuenta
     * @return mixed
     */
    public function actionDelete($cuenta)
    {
        $request = Yii::$app->request;
        $this->findModel($cuenta)->delete();
     ...
     /**
     * Delete multiple existing Bancos model.
     * For ajax request will return json object
     * and for non-ajax request if deletion is successful, the browser will be redirected to the 'index' page.
     * @param string $cuenta
     * @return mixed
     ...
     */
    protected function findModel($cuenta)
    {
        if (($model = Bancos::findOne(['cuenta' => $cuenta])) !== null) {
            return $model;
        } else {
