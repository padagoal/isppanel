...
use app\models\EstadocuentaBuscar;
...
class EstadocuentaController extends Controller
{
    ...
    {    
		...
		$searchModel = new EstadocuentaBuscar();
     ...
     * @return mixed
     */
    public function actionView($estadocuentaid)
    {   
        $request = Yii::$app->request; 		
        if($request->isAjax){
            Yii::$app->response->format = Response::FORMAT_JSON;
            return [
                    'title'=> Yii::t('app',$this->title)."# ".$estadocuentaid,
                    'content'=>$this->renderAjax('view', [
                        'model' => $this->findModel($estadocuentaid),
                    ]),
                    'footer'=> Html::button(Yii::t('app','Close'),['class'=>'btn btn-default pull-left','data-dismiss'=>"modal"]).
                            Html::a(Yii::t('app','Edit'),['update','estadocuentaid'=>$estadocuentaid],['class'=>'btn btn-primary','role'=>'modal-remote'])
                ];    
        }else{
        ...
        if($request->isAjax){
            ...
            */
            ...
            if($request->isGet){
                ...
                return [
                    ...
                    'title'=> Yii::t('app', "Add")." ".Yii::t('app', $this->title), 
                    'content'=>'<span class="text-success">Crear Estadocuenta realizado</span>',
                    'footer'=> Html::button(Yii::t('app', 'Close'),['class'=>'btn btn-default pull-left','data-dismiss'=>"modal"]).
            ...
            */
            if ($model->load($request->post()) && $model->save()) {
                return $this->redirect(['updatemd', 'estadocuentaid' => $model->estadocuentaid]);
            } else {
     ...
     * @return mixed
     */
    public function actionUpdate($estadocuentaid)
    {
        $request = Yii::$app->request;  		
        $model = $this->findModel($estadocuentaid);
		if ($model->hasProperty('created_by')) {
		...
		$model->vencimiento = Tools::dateFromdb($model->vencimiento); 
        if($request->isAjax){
            /*
            *   Process for ajax request
            */
            ...
            */
            if ($model->load($request->post()) && $model->save()) {
                return $this->redirect(['view', 'estadocuentaid' => $model->estadocuentaid]);
            } else {
    ...
    /**
     * Delete an existing Estadocuenta model.
     * For ajax request will return json object
     * and for non-ajax request if deletion is successful, the browser will be redirected to the 'index' page.
     * @param integer $estadocuentaid
     * @return mixed
     */
    public function actionDelete($estadocuentaid)
    {
        $request = Yii::$app->request;
        $this->findModel($estadocuentaid)->delete();
     ...
     /**
     * Delete multiple existing Estadocuenta model.
     * For ajax request will return json object
     * and for non-ajax request if deletion is successful, the browser will be redirected to the 'index' page.
     * @param integer $estadocuentaid
     * @return mixed
     ...
     */
    protected function findModel($estadocuentaid)
    {
        if (($model = Estadocuenta::findOne(['estadocuentaid' => $estadocuentaid])) !== null) {
            return $model;
        } else {
