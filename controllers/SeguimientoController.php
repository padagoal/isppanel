...
use app\models\SeguimientoBuscar;
...
class SeguimientoController extends Controller
{
    ...
    {    
		...
		$searchModel = new SeguimientoBuscar();
     ...
     * @param integer $seguimientoid
     * @return mixed
     */
    public function actionView($seguimientoid)
    {   
        $request = Yii::$app->request; 		
        if($request->isAjax){
            Yii::$app->response->format = Response::FORMAT_JSON;
            return [
                    'title'=> Yii::t('app',$this->title)."# ".$seguimientoid,
                    'content'=>$this->renderAjax('view', [
                        'model' => $this->findModel($seguimientoid),
                    ]),
                    'footer'=> Html::button(Yii::t('app','Close'),['class'=>'btn btn-default pull-left','data-dismiss'=>"modal"]).
                            Html::a(Yii::t('app','Edit'),['update','seguimientoid'=>$seguimientoid],['class'=>'btn btn-primary','role'=>'modal-remote'])
                ];    
        }else{
        ...
        if($request->isAjax){
            ...
            */
            ...
            if($request->isGet){
                ...
                return [
                    ...
                    'title'=> Yii::t('app', "Add")." ".Yii::t('app', $this->title), 
                    'content'=>'<span class="text-success">Crear Seguimiento realizado</span>',
                    'footer'=> Html::button(Yii::t('app', 'Close'),['class'=>'btn btn-default pull-left','data-dismiss'=>"modal"]).
            ...
            */
            if ($model->load($request->post()) && $model->save()) {
                return $this->redirect(['updatemd', 'seguimientoid' => $model->seguimientoid]);
            } else {
     ...
     * @return mixed
     */
    public function actionUpdate($seguimientoid)
    {
        $request = Yii::$app->request;  		
        $model = $this->findModel($seguimientoid);
		if ($model->hasProperty('created_by')) {
        ...
        if($request->isAjax){
            ...
            */
            if ($model->load($request->post()) && $model->save()) {
                return $this->redirect(['view', 'seguimientoid' => $model->seguimientoid]);
            } else {
    ...
    /**
     * Delete an existing Seguimiento model.
     * For ajax request will return json object
     * and for non-ajax request if deletion is successful, the browser will be redirected to the 'index' page.
     * @param integer $seguimientoid
     * @return mixed
     */
    public function actionDelete($seguimientoid)
    {
        $request = Yii::$app->request;
        $this->findModel($seguimientoid)->delete();
     ...
     /**
     * Delete multiple existing Seguimiento model.
     * For ajax request will return json object
     * and for non-ajax request if deletion is successful, the browser will be redirected to the 'index' page.
     * @param integer $seguimientoid
     * @return mixed
     ...
     */
    protected function findModel($seguimientoid)
    {
        if (($model = Seguimiento::findOne(['seguimientoid' => $seguimientoid])) !== null) {
            return $model;
        } else {
