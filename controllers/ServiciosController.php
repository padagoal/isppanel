...
use app\models\ServiciosBuscar;
...
class ServiciosController extends Controller
{
    ...
    {    
		...
		$searchModel = new ServiciosBuscar();
     ...
     * @return mixed
     */
    public function actionView($empresa, $servicio)
    {   
        $request = Yii::$app->request; 		
        if($request->isAjax){
            Yii::$app->response->format = Response::FORMAT_JSON;
            return [
                    'title'=> Yii::t('app',$this->title)."# ".$empresa, $servicio,
                    'content'=>$this->renderAjax('view', [
                        'model' => $this->findModel($empresa, $servicio),
                    ]),
                    'footer'=> Html::button(Yii::t('app','Close'),['class'=>'btn btn-default pull-left','data-dismiss'=>"modal"]).
                            Html::a(Yii::t('app','Edit'),['update','empresa, $servicio'=>$empresa, $servicio],['class'=>'btn btn-primary','role'=>'modal-remote'])
                ];    
        }else{
        ...
        if($request->isAjax){
            ...
            */
            ...
            if($request->isGet){
                ...
                return [
                    ...
                    'title'=> Yii::t('app', "Add")." ".Yii::t('app', $this->title), 
                    'content'=>'<span class="text-success">Crear Servicios realizado</span>',
                    'footer'=> Html::button(Yii::t('app', 'Close'),['class'=>'btn btn-default pull-left','data-dismiss'=>"modal"]).
            ...
            */
            if ($model->load($request->post()) && $model->save()) {
                return $this->redirect(['updatemd', 'empresa' => $model->empresa, 'servicio' => $model->servicio]);
            } else {
     ...
     * @return mixed
     */
    public function actionUpdate($empresa, $servicio)
    {
        $request = Yii::$app->request;  		
        $model = $this->findModel($empresa, $servicio);
		if ($model->hasProperty('created_by')) {
        ...
        if($request->isAjax){
            ...
            */
            if ($model->load($request->post()) && $model->save()) {
                return $this->redirect(['view', 'empresa' => $model->empresa, 'servicio' => $model->servicio]);
            } else {
    ...
    /**
     * Delete an existing Servicios model.
     * For ajax request will return json object
     ...
     * @return mixed
     */
    public function actionDelete($empresa, $servicio)
    {
        $request = Yii::$app->request;
        $this->findModel($empresa, $servicio)->delete();
     ...
     /**
     * Delete multiple existing Servicios model.
     * For ajax request will return json object
     * and for non-ajax request if deletion is successful, the browser will be redirected to the 'index' page.
     * @param string $empresa
     * @param string $servicio
     ...
     */
    protected function findModel($empresa, $servicio)
    {
        if (($model = Servicios::findOne(['empresa' => $empresa, 'servicio' => $servicio])) !== null) {
            return $model;
        } else {
