...
class ZonasController extends Controller
{
     ...
     * @return mixed
     */
    public function actionView($zona)
    {   
        $request = Yii::$app->request; 		
        if($request->isAjax){
            Yii::$app->response->format = Response::FORMAT_JSON;
            return [
                    'title'=> Yii::t('app',$this->title)."# ".$zona,
                    'content'=>$this->renderAjax('view', [
                        'model' => $this->findModel($zona),
                    ]),
                    'footer'=> Html::button(Yii::t('app','Close'),['class'=>'btn btn-default pull-left','data-dismiss'=>"modal"]).
                            Html::a(Yii::t('app','Edit'),['update','zona'=>$zona],['class'=>'btn btn-primary','role'=>'modal-remote'])
                ];    
        }else{
        ...
        if($request->isAjax){
            ...
            */
            ...
            if($request->isGet){
                ...
                return [
                    ...
                    'title'=> Yii::t('app', "Add")." ".Yii::t('app', $this->title), 
                    'content'=>'<span class="text-success">Crear Zonas realizado</span>',
                    'footer'=> Html::button(Yii::t('app', 'Close'),['class'=>'btn btn-default pull-left','data-dismiss'=>"modal"]).
            ...
            */
            if ($model->load($request->post()) && $model->save()) {
                return $this->redirect(['updatemd', 'zona' => $model->zona]);
            } else {
     ...
     * @return mixed
     */
    public function actionUpdate($zona)
    {
        $request = Yii::$app->request;  		
        $model = $this->findModel($zona);
		if ($model->hasProperty('created_by')) {
        ...
        if($request->isAjax){
            ...
            */
            if ($model->load($request->post()) && $model->save()) {
                return $this->redirect(['view', 'zona' => $model->zona]);
            } else {
     ...
     * @return mixed
     */
    public function actionDelete($zona)
    {
        $request = Yii::$app->request;
        $this->findModel($zona)->delete();
     ...
     */
    protected function findModel($zona)
    {
        if (($model = Zonas::findOne(['zona' => $zona])) !== null) {
            return $model;
        } else {
