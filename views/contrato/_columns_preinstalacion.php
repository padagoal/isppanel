...
return [
    ...
    [
        ...
        'value' => function ($model) {
            return  $model->getNameCliente($model->clienteid);
        },
    ],
    [
        'class'=>'\kartik\grid\DataColumn',
        'attribute'=>'vendedor',
        'value' => function ($model) {
            return  \app\models\Tools::nombreUsuario($model->vendedor);
        },
    ],
    [
        'class'=>'\kartik\grid\DataColumn',
...
'format' => ['date', 'php:d-m-Y'], 
    ...
    [
            ...
            'updatemd' => function ($url, $model) {
             ...
             'pago' => function ($url, $model) {
                 return Html::a('<span class="glyphicon glyphicon-briefcase"></span>', $url, [
                     'title' => Yii::t('app', 'Pago de Cuotas'),
                     'data-toggle'=>'tooltip',// 'role'=>'modal-remote',
                 ]);
             },
        ],
        'urlCreator' => function($action, $model, $key, $index) {
            if ($action === 'pago') {
                return Url::to(['/recibo/pagar', 'empresa' => $model->empresa,'clienteid' => $model->clienteid,'contrato'=>$model->contrato]);
            } else return Url::to([$action, 'empresa' => $model->empresa,'contrato' => $model->contrato]);//
        },
